...
type ColumnType = ColumnDef<ShopInterface, unknown>;
...
export function ShopListPage(props: ShopListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ShopInterface>>(
    () => `/shops?params=${JSON.stringify(params)}`,
    fetcher,
  );
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/shops/view/${record.id}`} passHref legacyBehavior>
            <Button
