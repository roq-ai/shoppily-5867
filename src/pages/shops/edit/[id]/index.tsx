...
import { getShopById, updateShopById } from 'apiSdk/shops';
...
function ShopEditPage() {
  ...
  const handleSubmit = async (values: ShopInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateShopById(id, values);
      mutate(updated);
      resetForm();
      router.push('/shops');
    } catch (error) {
